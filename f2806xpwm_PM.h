...
----------------------------------------------------------------------------*/
//jpw #define AQCTLA_INIT_STATE ( CAU_SET + CAD_CLEAR )
...
typedef struct {   
        ...
        Uint16 HalfPerMax;    // Parameter: Half of PeriodMax                   (Q0)    //jpw
//        int16 MfuncPeriod;    // Input: Period scaler (Q15)                             //jpw
        Uint16 Deadband;      // Parameter: PWM deadband in CPU clock cycles    (Q0)    //jpw
...
#define PWM_MACRO(m)                                                                        \
	...
	/*  from (-PeriodMax,PeriodMax) to (0,PeriodMax) where HalfPerMax=PeriodMax/2   */  \
	                                                                                    \
	     EPwm1Regs.CMPA.half.CMPA = _IQmpy(m.HalfPerMax,m.MfuncC1)+ m.HalfPerMax;    \
	     EPwm2Regs.CMPA.half.CMPA = _IQmpy(m.HalfPerMax,m.MfuncC2)+ m.HalfPerMax;    \
	     EPwm3Regs.CMPA.half.CMPA = _IQmpy(m.HalfPerMax,m.MfuncC3)+ m.HalfPerMax;    \
